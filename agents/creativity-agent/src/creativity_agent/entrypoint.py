...
class CreativityService:
    def __init__(self):
        ...
        self.http_client = httpx.AsyncClient(headers=self.headers, timeout=60.0)
    ...
    async def _send_request(
        ...
        try:
            response = await self.http_client.request(method, url, json=json_data, params=params)
            response.raise_for_status()
            response_json = response.json()
            logger.info(
                f"[Req ID: {request_id}] Received successful response from {url}. Status: {response.status_code}"
            )
            ...
            logger.debug(traceback.format_exc())
            raise HTTPException(status_code=503, detail=f"Could not connect to Creatify API: {e}")
        except Exception as e:
            logger.exception(f"[Req ID: {request_id}] Unexpected error during request to {url}: {e}")
            raise HTTPException(
                status_code=500, detail=f"Internal server error processing Creatify request: {type(e).__name__}"
            )
    ...
    async def handle_create_ai_short(self, request_body: AIShortRequest = Body(...)) -> dict:
        return await self._send_request(
            "POST", endpoint="/ai_shorts/", json_data=request_body.model_dump(exclude_unset=True)
        )

    @app.post(
        "/ai_shorts/preview/", response_model=AIShortResponse, tags=["AI Shorts"], summary="Generate AI Short Preview"
    )
    async def handle_generate_ai_short_preview(self, request_body: AIShortRequest = Body(...)) -> dict:
        return await self._send_request(
            "POST", endpoint="/ai_shorts/preview/", json_data=request_body.model_dump(exclude_unset=True)
        )

    @app.post(
        "/ai_shorts/{short_id}/render/",
        response_model=AIShortResponse,
        tags=["AI Shorts"],
        summary="Render AI Short Task",
    )
    ...
    @app.get(
        "/ai_shorts/{short_id}/", response_model=AIShortResponse, tags=["AI Shorts"], summary="Get AI Short Task Status"
    )
    ...
    async def handle_create_lipsync_task(self, request_body: LipsyncRequest = Body(...)) -> dict:
        return await self._send_request(
            "POST", endpoint="/lipsyncs/", json_data=request_body.model_dump(exclude_unset=True)
        )

    @app.post(
        "/lipsyncs/preview/",
        response_model=LipsyncPreviewResponse,
        tags=["Lipsync v1"],
        summary="Generate Lipsync v1 Preview",
    )
    async def handle_generate_lipsync_preview(self, request_body: LipsyncPreviewRequest = Body(...)) -> dict:
        return await self._send_request(
            "POST", endpoint="/lipsyncs/preview/", json_data=request_body.model_dump(exclude_unset=True)
        )

    @app.post(
        "/lipsyncs/{lipsync_id}/render/",
        response_model=StatusResponse,
        tags=["Lipsync v1"],
        summary="Render Lipsync v1 Task",
    )
    ...
    @app.get(
        ...
        tags=["Lipsync v1"],
        summary="Get Lipsync v1 Task Status",
    )
    ...
    async def handle_create_lipsync_v2_task(self, request_body: LipsyncV2Request = Body(...)) -> dict:
        return await self._send_request(
            "POST", endpoint="/lipsyncs_v2/", json_data=request_body.model_dump(exclude_unset=True)
        )

    @app.post(
        "/lipsyncs_v2/preview/",
        response_model=LipsyncV2Response,
        tags=["Lipsync v2"],
        summary="Generate Lipsync v2 Preview",
    )
    async def handle_generate_lipsync_v2_preview(self, request_body: LipsyncV2Request = Body(...)) -> dict:
        return await self._send_request(
            "POST", endpoint="/lipsyncs_v2/preview/", json_data=request_body.model_dump(exclude_unset=True)
        )

    @app.post(
        "/lipsyncs_v2/{lipsync_id}/render/",
        response_model=LipsyncV2Response,
        tags=["Lipsync v2"],
        summary="Render Lipsync v2 Task",
    )
    ...
    @app.get(
        "/lipsyncs_v2/{lipsync_id}/",
        response_model=LipsyncV2Response,
        tags=["Lipsync v2"],
        summary="Get Lipsync v2 Task Status",
    )
...
def agent_builder(args: dict[str, Any] | None = None) -> serve.handle.DeploymentHandle:
