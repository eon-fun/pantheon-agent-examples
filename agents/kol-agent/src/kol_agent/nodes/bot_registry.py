...
from kol_agent.models.raid_state import BotsModel, RaidState
...
ACTIONS = ["twitter_like", "twitter_comment", "twitter_retweet"]


def update_bot_actions(
    bots: list[BotsModel], raid_minutes: float, type_of_action: str, bots_actions: list[dict[str, Any]]
):
...
async def bot_registry(state: RaidState):
    ...
    bots_actions, message = plan_raid_actions(state["bot_accounts"], state["raid_minutes"])
    updated_state["bots_actions"] = bots_actions

    # Add message to log
    updated_state["messages"].append({"type": "bot_assignment", "content": message})
