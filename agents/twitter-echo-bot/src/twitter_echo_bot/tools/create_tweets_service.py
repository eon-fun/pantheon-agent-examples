...
class CreateTweetsService:
    ...
    async def create_tweet_for_user(self, user_data: PGUser, tweet_data: PGUserTweetMatch):
        async for session in get_db():
            ...
            await posted_tweets_manager.create_posted_tweet(
                user_id=user_data.id,
                source_tweet_id=tweet_data.tweet_id,
                source_account_id=orig_tweet_data.account_id,
                posted_text=text_for_tweet,
            )
            await post_tweet(
                access_token=await TwitterAuthClient.get_access_token(str(user_data.id)), tweet_text=text_for_tweet
            )
