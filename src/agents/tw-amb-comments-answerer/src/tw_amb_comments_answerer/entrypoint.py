...
class TwitterAmbassadorCommentsAnswerer(BaseAgent):
    ...
    async def answer_on_project_tweets_comments(
        ...
        db = get_redis_db()
        try:
            logger.info(f'answer_on_project_tweets_comments {my_username=} {project_username=} {keywords=} {themes=}')
            ...
            if not tweets_to_comment:
                logger.info("Nothing to comment. Already commented on all relevant tweets")
                return False
            ...
            for tweet in tweets_to_comment:
                if await check_answer_is_needed(tweet.full_text, my_username=my_username):
                    ...
                    if tweet_posted:
                        ...
                        logger.info(f'Posted reply to {tweet.id_str}: {comment_text}')

            return True
        except Exception as error:
            logger.error(f'answer_on_project_tweets_comments error: {my_username=} {error=}')
            raise
