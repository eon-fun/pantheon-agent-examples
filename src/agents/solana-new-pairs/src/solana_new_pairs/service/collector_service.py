...
from solana_new_pairs.service.dexscreener_service import DexScreenerAPI
...
async def collect_full_data_about_coin(coin_address: str):
    async for session in get_db():
        ...
        if coin_data is None:
            collector_data = await FullDataCollector(coin_address).collect_full_data()
            dex_tools_data = collector_data.get('dex_tools_data')
            print(dex_tools_data)
            ...
            if rug_check_data is None:
                rug_check_data = await FullDataCollector(coin_address).collect_data_from_rug_check()
