...
async def collect_full_data_about_coin(coin_address: str):
    async for session in get_db():
        ...
        if coin_data is None:
            collector_data = await FullDataCollector(coin_address, session,
                                                     base_coin_id=base_coin_id).collect_full_data()
            ...
            if rug_check_data is None:
                rug_check_data = await FullDataCollector(coin_address, session,
                                                         base_coin_id).collect_data_from_rug_check()
